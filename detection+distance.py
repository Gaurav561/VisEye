...
model_file = 'D:\BLIND\elan\elan\inference_graph'
      ...
      for i,b in enumerate(boxes[0]):
        ...
        if classes[0][i] ==1:
            if scores[0][i] >= 0.5:
                ...
                engine.say(apx_distance)
                engine.say("units")
#                 mytextperson = 'Person is AT A SAFER DISTANCE'
#                 myobj.save("person.mp3")
  
# # 
#                 os.system("person.mp3")
#                 engine.say("Person is AT A SAFER DISTANCE")
                ...
                if apx_distance <=0.5:
                    if mid_x > 0.3 and mid_x < 0.7:
                        ...
                        mytextperson = 'Warning -Person very close to the frame'
                        # myobj = gTTS(text=mytextperson, lang='en', slow=False)

                        # myobj.save("personnear.mp3")
  
# 
                        os.system("personnear.mp3")
                        # engine.say("Person is AT A SAFER DISTANCE")
...
#      plt.imshow(image_np)
    ...
    #   cv2.imshow('image',cv2.resize(image_np,(1024,768)))
      temp = cv2.resize(image_np,(1024,768))
    #   print(temp.shape)
    #   print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
      ##################################################
      if mm is None:
          mm = mmap.mmap(-1, temp.size, "IMG_NP")
        # write image
